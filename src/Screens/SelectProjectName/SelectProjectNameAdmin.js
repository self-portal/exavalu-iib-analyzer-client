...
let SelectProjectNameAdmin = () => {
  ...
  const navigateToUpload = () => {
    if (value) {
      if (location.state) {
        if (location.state.Menu === "jobParameterManagement") {
          navigate("/parametermangementspecific", {
            state: { jobName: value },
          });
        } else {
          navigate("/UploadProjects", { state: { jobName: value } });
        }
      } else {
        navigate("/UploadProjects", { state: { jobName: value } });
      }
  ...
  useEffect(() => {
    axios
      .get(baseURL, {
        headers: { "x-request-id": uuidv4(), is_filtered_sub_job: false },
      })
      .then((response) => {
      ...
      <div class="openjob-container-fluid">
        <div id="addsubjob-border-box" className="mt-5 ">
          <div>
            ...
            <div class="d-flex justify-content-center" id="scan-job-submit">
              <button
                disabled={!value}
                variant="contained"
...
export default SelectProjectNameAdmin;
